...
def display_pending(service_records):
    ...
    for record in service_records:
        student_name = f"{record.student.get_name()} (ID: {record.student.id})"
...
def display_leaderboard(leaderboard):
    ...
    for student in leaderboard:
        rank += 1
        student_name =  f"{student['student'].get_name()} (ID: {student['student'].id})"
        hours = student["hours"]
...
def display_accolade_unlocked(accolade_records):
    if not accolade_records:
        return False
    
    student_name = accolade_records[0].student.get_name()

    for record in accolade_records:
        accolade = record.accolade
        print(f"Accolade Unlocked - {student_name} just earned [{accolade.title}] for {accolade.target_hours} hours of service!")
